...
export default function ResponseList({
  ...
  const [columns, setColumns] = useState([{ field: 'Action', headerName: 'Action' }]);
  ...
  useEffect(() => {
    {
      form?.fields?.map((field, i) =>
        setColumns((arr) => [...arr, { field: field.label, headerName: field.label }]),
      );
    }
  }, []);
  ...
  return (
    <>
      ...
      ) : form?.settings?.responsesView === 'MuiDataGrid' ? (
        <DataGrid pageSize={data?.getResponses?.count || 0} rowsPerPageOptions={[10, 25, 50]} />
      ) : form?.settings?.responsesView === 'reactDataSheet' ? (
        <Example />
      ) : form?.settings?.responsesView === 'reactTable' ? (
        <App />
      ) : (
        // ) : form?.settings?.responsesView === 'reactSpreadSheet' ? (
        //   <UsersGrid/>
        //   // ""
        <>
