            ...
            if (typeof src !== typeof undefined && src !== false) {
                if (src.indexOf('base64') !== -1) {
                    ...
                    if(this.opts.onBase64Upload) {
                        ...
                        let w = img.getAttribute('data-width');
                        let h = img.getAttribute('data-height');

                        this.opts.onBase64Upload(img, image, filename, w, h); // target image, base64 string, filename
                        ...
                        if(!this.builderStuff.querySelector('#form-' + this.count)) {
                            var html = '<form id="form-' + this.count + '" target="frame-' + this.count + '" method="post" enctype="multipart/form-data">' +
                                ...
                                '<input id="hidwidth-' + this.count + '" name="hidwidth-' + this.count + '" type="hidden" />' +
                                '<input id="hidheight-' + this.count + '" name="hidheight-' + this.count + '" type="hidden" />' +
                        ...
                        //Additional width & height info
                        let w = img.getAttribute('data-width');
                        if(w) this.builderStuff.querySelector('#hidwidth-' + this.count).value = w;
                        let h = img.getAttribute('data-height');
                        if(h) this.builderStuff.querySelector('#hidheight-' + this.count).value = h;
