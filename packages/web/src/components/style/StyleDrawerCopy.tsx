...
export default function StyleDrawer({
  ...
  const handleTitleChange = (e, isNumber?: boolean) => {
    const name = e?.target?.name;
    let value = e?.target?.value;
    if (isNumber) {
      value = value ? Number(value) : null;
  ...
  const handleDesChange = (e, isNumber?: boolean) => {
    const name = e?.target?.name;
    let value = e?.target?.value;
    if (isNumber) {
      value = value ? Number(value) : null;
  ...
  console.log(title, 'This is the titiel');
  return (
    <Drawer
      ...
      <div style={{ minWidth: 300 }} className="p-2">
        ...
        <InputGroup>
          <TextField
            ...
            fullWidth
            name="padding"
            onChange={(e) => handleDesChange(e, false)}
            value={styles.padding}
